 ...
 * every group. They are trusted entries, so they use the allowlist's explicit
 * `path`/`containerName` pair instead of the user-requested additional-mount
 * validation path, whose generic blocked-pattern rules would reject intentional
 * mounts like the credentials service directory.
 */
