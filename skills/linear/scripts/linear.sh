...
resolve_project_id() {
  ...
  gql "{ projects(first: 50, filter: { name: { containsIgnoreCase: \\\"$project_name\\\" } }) { nodes { id name } } }" \
    | jq -r '.data.projects.nodes[0].id // empty'
}

resolve_label_ids_csv() {
  local team_id="$1"
  local labels_csv="$2"
  ...
  for raw in "${labels[@]}"; do
    ...
    [[ -z "$label" ]] && continue
    local id
    id=$(gql "{ issueLabels(first: 50, filter: { team: { id: { eq: \"$team_id\" } }, name: { eq: \"$label\" } }) { nodes { id name } } }" | jq -r '.data.issueLabels.nodes[0].id // empty')
    if [[ -z "$id" ]]; then
      echo "Warning: label not found for team: $label" >&2
    else
      ids+=("\"$id\"")
    fi
  done
...
get_viewer_id() {
  gql "{ viewer { id } }" | jq -r '.data.viewer.id // empty'
}
...
get_state_id() {
  ...
  case "$state_name" in
    ...
    blocked|cancelled|canceled) state_name="Canceled" ;;
...
"'
  ...
  create)
    ...
    if [[ -z "$team_key" || -z "$title" ]]; then
      echo "Usage: linear.sh create <TEAM_KEY> \"Title\" [\"Description\"] [--project <name>] [--labels <a,b>] [--priority <urgent|high|medium|low|none>] [--state <todo|progress|review|done|blocked|backlog>] [--assignee me] [--source <company-relative-path>]" >&2
      exit 1
    fi
    ...
    if [[ -n "$project_name" ]]; then
      project_id=$(resolve_project_id "$team_id" "$project_name")
      if [[ -n "$project_id" ]]; then
        project_field=", projectId: \\\"$project_id\\\""
      else
        echo "Warning: project not found for team ($team_key): $project_name" >&2
      fi
    fi

    labels_field=""
    if [[ -n "$labels_csv" ]]; then
      label_ids=$(resolve_label_ids_csv "$team_id" "$labels_csv")
      if [[ -n "$label_ids" ]]; then
        labels_field=", labelIds: [$label_ids]"
      fi
    fi
    ...
    if [[ -n "$priority_name" ]]; then
      case "$priority_name" in
        ...
        low) pval=4 ;;
        none) pval=0 ;;
        *) echo "Unknown priority: $priority_name" >&2; exit 1 ;;
      esac
      priority_field=", priority: $pval"
    fi

    state_field=""
    if [[ -n "$state_name" ]]; then
      state_id=$(get_state_id "$team_key" "$state_name")
      if [[ -n "$state_id" && "$state_id" != "null" ]]; then
        state_field=", stateId: \\\"$state_id\\\""
      else
        echo "Warning: state not found for team ($team_key): $state_name" >&2
      fi
    fi

    assignee_field=""
    if [[ "$assignee_mode" == "me" ]]; then
      viewer_id=$(get_viewer_id)
      if [[ -n "$viewer_id" ]]; then
        assignee_field=", assigneeId: \\\"$viewer_id\\\""
      fi
    fi
    ...
    if [[ -n "$source_path" ]]; then
      if [[ -n "$description" ]]; then
        ...
        description="Source: $source_path"
      fi
    fi
    ...
    result=$(gql "mutation { issueCreate(input: { teamId: \\\"$team_id\\\", title: \\\"$title\\\", description: \\\"$description\\\"$project_field$labels_field$priority_field$state_field$assignee_field }) { success issue { identifier title url } } }")
  ...
  help|*)
    ...
    echo "  create <TEAM_KEY> \"title\" [\"desc\"] [--project <name>] [--labels <a,b>] [--priority <level>] [--state <state>] [--assignee me] [--source <path>]"
