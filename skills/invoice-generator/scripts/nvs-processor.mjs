...
async function processArEmails(dryRun) {
  ...
  // Also catch forwarded emails from Cian's NVS alias (AR subjects: "Invoice for Rustam")
  const directQuery = `from:${processing.ar.sender} -label:${PROCESSED_LABEL} -label:${SKIPPED_LABEL} -label:${NEEDS_REVIEW_LABEL}`;
  const forwardedQuery = `from:cian.whalley@newvaluegroup.com subject:"Invoice for Rustam" -label:${PROCESSED_LABEL} -label:${SKIPPED_LABEL} -label:${NEEDS_REVIEW_LABEL}`;
  
  const directResult = await gmail.searchMessages(directQuery, 10);
  const forwardedResult = await gmail.searchMessages(forwardedQuery, 10);
  
  const messages = [];
  const seenIds = new Set();
  for (const msg of [...(directResult.messages || []), ...(forwardedResult.messages || [])]) {
  ...
  for (const msg of messages) {
    ...
    // Skip forwarded emails that don't actually contain AR content
    const isArForward = parsed.body.includes(processing.ar.sender);
    const isForwarded = parsed.subject.toLowerCase().startsWith('fwd:');
    if (isForwarded && !isArForward) {
      console.log(`      ⏭️  Forwarded email does not contain AR content. Skipping.`);
      continue;
    }
...
async function processApEmails(dryRun) {
  ...
  const labelId = await gmail.getOrCreateLabel(PROCESSED_LABEL);
  const reviewId = await gmail.getOrCreateLabel(NEEDS_REVIEW_LABEL);
  // Also catch forwarded emails from Cian's NVS alias (AP subjects: "PO" or "Purchase Order")
  const directQuery = `from:${processing.ap.sender} -label:${PROCESSED_LABEL} -label:${SKIPPED_LABEL} -label:${NEEDS_REVIEW_LABEL}`;
  const forwardedQuery = `from:cian.whalley@newvaluegroup.com subject:PO -label:${PROCESSED_LABEL} -label:${SKIPPED_LABEL} -label:${NEEDS_REVIEW_LABEL}`;
  ...
  for (const msg of messages) {
    ...
    // Skip forwarded emails that don't actually contain AP content
    const isApForward = parsed.body.includes(processing.ap.sender);
    const isForwarded = parsed.subject.toLowerCase().startsWith('fwd:');
    if (isForwarded && !isApForward) {
      console.log(`      ⏭️  Forwarded email does not contain AP content. Skipping.`);
      continue;
    }
    ...
    // NVS issues POs in the month AFTER the work period (e.g. April work → PO issued May 1)
    // Invoice date = 1st of PO issue month, due = last day of that month
    let invoiceMonth = period.month + 1;
    let invoiceYear = period.year;
