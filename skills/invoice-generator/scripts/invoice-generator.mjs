...
function parseArgs() {
  ...
  const validClients = ['copperteams', 'ganttsy', 'work-wranglers', 'kevin-lee', 'nvs', 'athyna-insight', 'all'];
...
async function exportTogglPdf(projectId, since, until, clientName) {
  ...
  const TEMPLATE_MAP = {
    ...
    '219996573': 'athyna.json'   // Athyna Insight
  };
...
/**
 * Generate Athyna Insight invoice (ad-hoc hours)
 */
async function generateAthynaInsightInvoice(year, month) {
  console.log('\n📋 Generating Athyna Insight invoice...');
  const { since, until, priorMonth, priorYear } = getDateRange(year, month);
  const clientConfig = config.clients['athyna-insight'];
  const projectConfig = config.projects['athyna-insight'];
  ...
  const lineItems = [];
  const period = priorYear + '-' + String(priorMonth).padStart(2, '0');
  lineItems.push({ itemCode: clientConfig.itemCode, description: 'Fractional CTO - Ad-Hoc Consulting (' + period + ')', quantity: hours.totalHours, unitAmount: clientConfig.excessRate, taxType: projectConfig.taxType, accountCode: projectConfig.accountCode });

  const invoice = await xero.createDraftInvoice(clientConfig.contactName, lineItems, 'Regulatory Intelligence Pilot - ' + year + '-' + String(month).padStart(2, '0'), month, year);
  await attachTogglPdf(invoice, 'Athyna Insight', projectConfig.id, since, until);
  return invoice;
}
...
async function main() {
  ...
  let clientsToProcess = client === 'all' ? ['work-wranglers', 'copperteams', 'ganttsy', 'kevin-lee', 'nvs', 'athyna-insight'] : [client];
  ...
  for (const clientName of clientsToProcess) {
    try {
      switch (clientName) {
        ...
        case 'athyna-insight': results[clientName] = await generateAthynaInsightInvoice(year, month); break;
      }
    } catch (error) {
