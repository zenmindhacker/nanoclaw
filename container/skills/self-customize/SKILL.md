...
- **`CLAUDE.local.md` or files in your workspace** → Edit directly, no approval needed. Your workspace (`/workspace/agent/`) is persisted on the host for channel-specific overrides. (Note: the composed `CLAUDE.md` itself is read-only and regenerated every spawn — write to `CLAUDE.local.md` instead.)
- **Agent-wide personality or wiki** → Edit `/workspace/global/CLAUDE.local.md` or `/workspace/global/wiki/` directly. Shared across all groups; no approval needed for markdown/memory edits. Persistence rules live in `container/CLAUDE.md`; see `docs/claude-md-composition.md` in the repo.
