...
const CYCLE_START = new Date('2026-05-27T00:00:00-04:00');
...
if (process.argv.includes('--task-json')) {
  const dateArg = resolveDateArg(process.argv);
  const briefing = getDailyBriefing(dateArg);
  console.log(
    JSON.stringify({
      wakeAgent: true,
      data: { date: briefing.date, text: formatBriefing(briefing) },
    }),
  );
} else {
